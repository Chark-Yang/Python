"""
显示一个UR5e机械臂在Mujoco中执行关节空间轨迹的，实时可视化接触力的方向和大小。
"""
...
class JointSpaceTrajectory:
    ...
    def get_next_waypoint(self, qpos):
        ...
        # np.allclose(a, b, atol=0.02)：逐元素比较数组 a 和 b，若对于所有元素都有 |a - b| <= atol + rtol * |b|（默认 rtol=1e-08），
        # 则返回 True。这里用 atol=0.02 判断“足够接近”。
...
def main():
    model = mujoco.MjModel.from_xml_path('model/mujoco_menagerie/universal_robots_ur5e/scene.xml')
