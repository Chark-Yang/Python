"""
仿真环境下,ur5e+graspnt+二指夹爪实现物体的抓取与搬运，
运行主程序 main.py
...
if __name__ == '__main__':
    ...
    # q1 = np.array([0.0, 0.0, np.pi / 3, 0.0, -np.pi / 3, 0.0])
    ...
    # print(type(T2.R))
    # print("T2.R:\n", T2.R)
    # print("det T1:", np.linalg.det(T1.R))
    # print("det T2:", np.linalg.det(T2.R))
    # R0 = sm.SO3(T1.R,check=False)
    # print(type(R0))
    # print(R0)
    # R1 = sm.SO3(T2.R,check=False)
    # print(type(R1))
    # print(R1)
    ...
    attitude_parameter1 = OneAttitudeParameter(sm.SO3(T1.R, check=False).norm(), sm.SO3(T2.R, check=False).norm())
    ...
    # print(type(trajectory_parameter1))
    # print(type(sm.SO3(T1.R, check=False).norm()))
    # print(type(sm.SO3(T2.R, check=False).norm()))
    ...
    attitude_parameter2 = OneAttitudeParameter(sm.SO3(T2.R, check=False).norm(), sm.SO3(T3.R, check=False).norm())
        ...
        for j in range(len(time_cumsum)):
            if timei < time_cumsum[j]:
                ...
                if isinstance(planner_interpolate, np.ndarray):
                    ...
                    # print("target:", planner_interpolate.t)
                    # print(type(planner_interpolate))
                    # 错误示例
                    # robot.move_cartesian(planner_interpolate)
                    # robot.move_cartesian(sm.SE3(planner_interpolate.t))
                    #robot.move_cartesian(sm.SE3(planner_interpolate.R, planner_interpolate.t))
                    robot.move_cartesian(sm.SE3(planner_interpolate))
                    joint = robot.get_joint()
                    # if not robot.move_cartesian(planner_interpolate):
                    #     continue   # 跳过这个时间步
                    # joint = robot.get_joint()
                action[:6] = joint
    ...
    # attitude_parameter3 = OneAttitudeParameter(sm.SO3(T3.R).norm(), sm.SO3(T4.R).norm())
    # attitude_parameter3 = OneAttitudeParameter(sm.SO3(T3.R, check=False).norm(), sm.SO3(T4.R, check=False).norm())
    attitude_parameter3 = OneAttitudeParameter(sm.SO3(T3.R, check=False).norm(), sm.SO3(T4.R, check=False).norm())
    ...
    # T4 -> T5(移动物体到目标位置上方)
    time4 = 2
    T5 = sm.SE3.Trans(1.4, 0.2, T4.t[2]) * sm.SE3(sm.SO3(T4.R,check=False).norm())
    position_parameter4 = LinePositionParameter(T4.t, T5.t)
    attitude_parameter4 = OneAttitudeParameter(sm.SO3(T4.R, check=False).norm(), sm.SO3(T5.R, check=False).norm())
    ...
    # T5 -> T6(下降到目标位置)
    time5 = 2
    T6 = sm.SE3.Trans(0.2, 0.2, T5.t[2]) * sm.SE3(sm.SO3.Rz(-np.pi / 2) * sm.SO3(T5.R,check=False).norm())
    position_parameter5 = LinePositionParameter(T5.t, T6.t)
    attitude_parameter5 = OneAttitudeParameter(sm.SO3(T5.R, check=False).norm(), sm.SO3(T6.R, check=False).norm())
    ...
    attitude_parameter6 = OneAttitudeParameter(sm.SO3(T6.R, check=False).norm(), sm.SO3(T7.R, check=False).norm())
