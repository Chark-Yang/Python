...
from manipulator_grasp.utils import mj_l20 as mj
import ikpy.chain


class jakaGraspEnv_l20:
    ...
    def reset(self):
        ...
        MUJOCO_XML_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'assets', 'scenes', 'scene_jaka_l20.xml')
        ...
        # 挂载夹爪，正向运动学求解，计算末端位姿
        mj.attach(self.mj_model, self.mj_data, "attach", "l20", sm.SE3(T_gripper_base))
        print("ncon =", self.mj_data.ncon)

        for i in range(min(20, self.mj_data.ncon)):
            c = self.mj_data.contact[i]

            print(
                i,
                self.mj_model.geom(c.geom1).name,
                self.mj_model.geom(c.geom2).name
            )
