...
class QNetwork(nn.Module):
    ...
    def get_action(self, x, action=None):
        ...
        # view(len(x), self.n, self.n_atoms)把102 个数字变成一个 2 行 51 列的矩阵。
        # torch.softmax(..., dim=2)把每行的51个数字变成一个概率分布，表示每个atom的概率
        ...
        if global_step > args.learning_starts:
            if global_step % args.train_frequency == 0:
                ...
                with torch.no_grad():
                    ...
                    # projection；计算间距；强制限制边界；计算上下界；floor向下取整，ceil向上取整
                    ...
                    # + (l == u).float()是为防止b刚好是整数，b == l == u,如果不加这个项，都为0
                    d_m_l = (u + (l == u).float() - b) * next_pmfs
                    d_m_u = (b - l) * next_pmfs
                    target_pmfs = torch.zeros_like(next_pmfs)
                    # l u d_m_l 和 d_m_u 都是batch_size * n_actions的矩阵
