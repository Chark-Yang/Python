...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        # PPO连续控制特点：actor网络被拆成了2部分，输出均值(Mean)和独立的标准差参数(Log_std)
        ...
        # 对数标准差参数，保证标准差为正数；
        # nn.Parameter会被自动添加到模型的参数列表中，并且在训练过程中会被更新，让探索率成为可自动衰减的全局变量
        # zeros保证初始值标准差为1，初始探索非常充分
        self.actor_logstd = nn.Parameter(torch.zeros(1, np.prod(envs.single_action_space.shape)))

    def get_value(self, x):
        return self.critic(x)

    # 数据收集阶段：没有传入action，随机选取动作
    # 网络更新阶段：此时参数可能已经更新过了，传入action，评估旧动作在新策略下的概率，从而计算概率比值
    # 在连续控制中，一个动作不是分立的0或者1，而是一个浮点数；对于一个具有16个自由度的灵巧手，神经网络输出的action是包含16个浮点数的向量，对应的probs.log_prob(action).sum(1)计算是16个独立事件同时发生的概率，按理应该是16个概率的乘积，取对数后就是16个概率的和，所以这里用了.sum(1)
    def get_action_and_value(self, x, action=None,deterministic=False):
        # PPO连续控制特点：动作抽样来自正态分布
        ...
        if action is None:
            if deterministic:
                # 【核心修改】：如果是测试模式，直接输出高斯分布的均值（最高概率点）
                action = action_mean
            else:
                # 训练模式，依然保持抽样以维持探索
                action = probs.sample()
...
if __name__ == "__main__":
    ...
    best_episodic_return = -np.inf  # 记录历史最高分
